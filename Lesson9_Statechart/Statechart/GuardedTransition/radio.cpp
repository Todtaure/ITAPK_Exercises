...
{
	EvCDState(bool valid_cd) : valid_cd_(valid_cd)	{	}
	bool getValidCd() const
	{
		return valid_cd_;
	}
...
struct CDLoading : sc::simple_state<CDLoading, On>
{
	...
	/* Missing code */
	typedef boost::mpl::list<sc::custom_reaction<EvCDState>, sc::custom_reaction<EvEject> > reactions;
	...
	sc::result react(const EvEject &ev)
	{
		return transit<RadioPlaying>();
	}
